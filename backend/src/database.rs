...
pub fn list_videos(cnx: &PooledConnection<SqliteConnectionManager>) -> rusqlite::Result<Vec<Video>> {
    let mut stmt = cnx.prepare("SELECT name, url, tags, original, thumbnail, creation_timestamp FROM videos")?;
...
pub fn add_tag(cnx: &PooledConnection<SqliteConnectionManager>, video_name: &str, tags: &Vec<String>) -> rusqlite::Result<()> {
...
mod tests {
    use r2d2_sqlite::SqliteConnectionManager;
    use crate::DbPool;
    use crate::database::{create_database, insert_video, Video, list_videos, add_tag};
    use std::borrow::Borrow;
    ...
    fn test_list_videos() {
        ...
        assert_eq!(list_videos(pool.get().unwrap().borrow()).expect("cannot list videos").len(), 1);
    }
    ...
    fn test_add_tags() {
        ...
        add_tag(pool.get().unwrap().borrow(),
                video.name.as_str(),
                vec![String::from("tags")].borrow()).expect("cannot add tag");
        let vids = list_videos(pool.get().unwrap().borrow()).expect("cannot list videos");
        assert_eq!(vids.len(), 1);
        assert_eq!(vids.get(0).unwrap().tags.len(), 1);
        assert_eq!(vids.get(0).unwrap().tags.get(0).unwrap(), "tags");
    }
}